...
Public Class ResponsiveHelper
    ...
    Public Shared Sub ScaleForm(form As Form, baseWidth As Integer, baseHeight As Integer)
        ...
        ' Enforce minimum dialog size based on screen size
        ' Use 50% of screen as minimum, but at least 500x400
        Dim minWidth = Math.Max(500, CInt(_screenWidth * 0.5))
        Dim minHeight = Math.Max(400, CInt(_screenHeight * 0.5))
        
        If scaledWidth < minWidth Then scaledWidth = minWidth
        If scaledHeight < minHeight Then scaledHeight = minHeight
        
        ' Don't exceed screen bounds (use 95% to maximize space)
        ...
        Debug.WriteLine($"[SCALE FORM] {form.Text}: Base={baseWidth}x{baseHeight}, Scaled={scaledWidth}x{scaledHeight}, Screen={_screenWidth}x{_screenHeight}")
