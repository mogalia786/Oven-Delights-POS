...
Public Class CakeOrderEditService
    ...
    Public Sub StartEditWorkflow()
        Try
            ...
            ' Step 4: Open CakeOrderFormNew in EDIT mode
            If selectedOrderID > 0 Then
                Dim editForm As New CakeOrderFormNew(_branchID, _tillPointID, _cashierID, _cashierName, _branchName, _branchAddress, _branchPhone, Nothing, selectedOrderID)
                editForm.ShowDialog()
                editForm.Dispose()
            End If

        Catch ex As Exception
            MessageBox.Show($"Error in edit workflow: {ex.Message}{vbCrLf}{vbCrLf}Stack Trace:{vbCrLf}{ex.StackTrace}", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error)
        End Try
    End Sub
End Class
